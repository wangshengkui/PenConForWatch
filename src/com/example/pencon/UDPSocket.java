...
/*    private HeartbeatTimer timer;
    ...
    public UDPSocket(MainActivity context) {
...
//        startHeartbeatTimer();
    ...
    private void receiveMessage() {
        while (isThreadRunning) {
            try {
                if (client != null) {
                    client.receive(receivePacket);//将client接收的数据放入receivePacket
                }
                lastReceiveTime = System.currentTimeMillis();
...
//            mContext.mHandler.obtainMessage(MainActivity.MESSAGE_READ,receivePacket.getData(),-1, buffer).sendToTarget();
//            String strReceive = new String(receivePacket.getData(), 0, receivePacket.getLength());
            Log.d(TAG,  " 信息来自： " + receivePacket.getAddress().getHostAddress() + ":" + receivePacket.getPort());
...
*/
     ...
     * @param message
     */
    public void sendMessage(final byte[] message ,final String ipString) {
        mThreadPool.execute(new Runnable() {
            @Override
            public void run() {
                try {
//                    InetAddress targetAddress = InetAddress.getByName(BROADCAST_IP);
                	InetAddress targetAddress = InetAddress.getByName(ipString);
                    DatagramPacket packet = new DatagramPacket(message, message.length, targetAddress, CLIENT_PORT);
